...
public class MyGui : MonoBehaviour
{
  ...
  private void OnGUI()
  {
    if (!UseGui)
      return;
    if (GUI.Button(new Rect(10, 15, 105, 30), "Effet Précédent")) {
      ChangeCurrent(-1);
    }
    if (GUI.Button(new Rect(130, 15, 105, 30), "Effet Suivant"))
    {
      ChangeCurrent(+1);
    }
    ...
    if (current <= 15) {
      ...
      isHomingMove = GUI.Toggle(new Rect(10, 190, 150, 30), isHomingMove, " Suivre la cible");
